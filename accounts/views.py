...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            pw_check_start = time.time()
            if not user.check_password(password):
                messages.error(request, "Password is incorrect.")
                logger.info("Login failed: bad password (email=%s)", email)
                return render(request, "login.html", {"form": form})
            ...
            logger.info(
                "Login timing (email=%s): db=%.3fs pw=%.3fs face=%.3fs total=%.3fs",
                email, db_lookup_time, pw_check_time, face_time, total_time,
            )
...
def lostpassword_view(request):
    ...
    if request.method == "POST":
        ...
        try:
            send_mail(
                "NocheatZone - Password Reset OTP",
...
@login_required
@require_GET
def growth_rate_view(request):
    """Render student growth rate analytics page (simple render of template)."""
    if request.user.user_type != "student":
        return redirect("professor_index")
    return render(request, "growth_rate.html")
