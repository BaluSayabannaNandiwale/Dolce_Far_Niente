...
$(document).ready(function () {
  ...
  console.log('Subjective - Raw time element text:', timeText);
  
  var time = parseInt(timeText);
  console.log('Subjective - Parsed time (seconds):', time);
  
  if (!isNaN(time) && time > 0) {
    console.log('Subjective - Starting timer with', time, 'seconds');
    ...
    console.warn('Subjective - Invalid time value:', timeText);
...
function startTimer(duration, display) {
  ...
  console.log('Subjective Timer started with duration:', duration, 'seconds');
  ...
  timerInterval = setInterval(function () {
    ...
    if (timer % 30 === 0 || timer < 10) {
      console.log('⏱️ Subjective Timer:', timeString, '(remaining:', timer, 's)');
    }
    
    if (--timer < 0) {
      console.log('⏰ Subjective Timer expired - auto-submitting exam');
      clearInterval(timerInterval);
      timerInterval = null;
      flag_time = false;
      submitformexam();
  ...
  console.log('✓ Subjective timer interval started successfully');
}
